...
pub async fn spawn(cmd: CommandBuilder) -> io::Result<UnixPtyProcess> {
    ...
    let cwd_cstr: Option<CString> = cmd
        .cwd
        .as_deref()
        .map(|p| to_cstring(p.to_str().unwrap_or("")))
        .transpose()?;
    ...
    match fork_res {
        ...
        child_pid => {
            ...
            let n =
                unsafe { libc::read(pipe_read, errno_buf.as_mut_ptr() as *mut libc::c_void, 4) };
